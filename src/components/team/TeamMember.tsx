        ...
        >
          <div className="relative overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              ...
              <p className="text-gold-200 text-lg">{member.role}</p>
            </div>
          </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {member.specialties.slice(0, 3).map((specialty, index) => (
                  <span 
                    key={index} 
                    className="bg-gold-100 text-gold-700 text-xs px-2 py-1 rounded-full"
                  >
                    {specialty}
                  </span>
                ))}
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            <div className="md:w-1/3">
              <Avatar className="w-32 h-32 md:w-48 md:h-48 rounded-lg">
                ...
                <AvatarFallback className="text-2xl bg-gold-200 text-gold-700">
                  {getInitials(member.name)}
                </AvatarFallback>
              </Avatar>
            </div>
            <div className="md:w-2/3">
              <h2 className="text-3xl font-serif font-medium text-gold-800 mb-2">
                {member.name}
              </h2>
              <p className="text-lg text-gold-500 mb-4">{member.role}</p>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {member.specialties.map((specialty, index) => (
                    <span 
                      key={index} 
                      className="bg-gold-100 text-gold-700 px-3 py-1 rounded-full"
                    >
                      {specialty}
                    </span>
                  ))}
              ...
              <div className="mt-8">
                <a 
                  href="/booking" 
                  className="inline-block bg-gold-500 text-white px-5 py-2 rounded-full hover:bg-gold-600 transition-colors"
                >
